...
import { QUIZ_QUESTIONS } from '@/data/quiz-l1-thesis'
...
export function QuizPlayer() {
  ...
  const score = answers.filter((a, i) => a === QUIZ_QUESTIONS[i].correctAnswer).length
  const pct = Math.round((score / total) * 100)

  const getGrade = () => {
    if (pct >= 90) return { grade: 'A+', msg: 'Outstanding! You are fully exam-ready!', color: 'text-green-600 dark:text-green-400' }
    if (pct >= 80) return { grade: 'A', msg: 'Excellent! Strong understanding of all concepts.', color: 'text-green-600 dark:text-green-400' }
    if (pct >= 70) return { grade: 'B', msg: 'Good! Review the questions you missed.', color: 'text-blue-600 dark:text-blue-400' }
    if (pct >= 60) return { grade: 'C', msg: 'Passing — but more revision needed before exam day.', color: 'text-yellow-600 dark:text-yellow-400' }
    return { grade: 'D', msg: 'Needs more study. Re-read the chunks you struggled with.', color: 'text-red-600 dark:text-red-400' }
  }
  ...
  if (showResult) {
    const { grade, msg, color } = getGrade()
      ...
      <div className="max-w-3xl mx-auto py-8">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-10 text-center">
            ...
            <div className="text-2xl text-indigo-100 mb-3">{pct}% Correct</div>
            <p className="text-indigo-100 text-lg">{msg}</p>
          </div>
    ...
    <div className="max-w-3xl mx-auto py-8">
      ...
      <div className="mb-6">
        <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400 mb-2">
          <span className="font-medium text-indigo-600 dark:text-indigo-400">{q.chunk}</span>
        ...
        <div className="flex justify-between text-xs text-gray-400 mt-1">
          <span>
            Score so far: {answers.filter((a, i) => a === QUIZ_QUESTIONS[i].correctAnswer).length}/
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden mb-4">
        <div className="px-6 py-5 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-3">
            <span className="bg-indigo-600 text-white text-xs font-bold px-2.5 py-1 rounded-full">Q{q.id}</span>
          </div>
