...
import { OPERATING_LAYER_QUESTIONS } from '@/data/quiz-l1-operating-layer'

export function QuizPlayerOperatingLayer() {
  const [current, setCurrent] = useState(0)
  const [selected, setSelected] = useState<string | null>(null)
  const [answers, setAnswers] = useState<(string | null)[]>(Array(OPERATING_LAYER_QUESTIONS.length).fill(null))
  const [showResult, setShowResult] = useState(false)
  const [confirmed, setConfirmed] = useState(false)

  const q = OPERATING_LAYER_QUESTIONS[current]
  const total = OPERATING_LAYER_QUESTIONS.length
  ...
  const handleRestart = () => {
    setCurrent(0)
    setSelected(null)
    setAnswers(Array(OPERATING_LAYER_QUESTIONS.length).fill(null))
    setShowResult(false)
    setConfirmed(false)
  }

  const score = answers.filter((a, i) => a === OPERATING_LAYER_QUESTIONS[i].correctAnswer).length
  const pct = Math.round((score / total) * 100)

  const getGrade = () => {
    if (pct >= 90) return { grade: 'A+', msg: 'Outstanding! Full command of the AI Operating Layer.', color: 'text-green-600 dark:text-green-400' }
    if (pct >= 80) return { grade: 'A', msg: 'Excellent! Strong understanding of all key concepts.', color: 'text-green-600 dark:text-green-400' }
    if (pct >= 70) return { grade: 'B', msg: 'Good! Review the questions you missed before the exam.', color: 'text-blue-600 dark:text-blue-400' }
    if (pct >= 60) return { grade: 'C', msg: 'Passing — but revise the Operating Layer notes once more.', color: 'text-yellow-600 dark:text-yellow-400' }
    return { grade: 'D', msg: 'Needs more study. Re-read the notes and try again.', color: 'text-red-600 dark:text-red-400' }
  }

  const chunkScores = () => {
    const chunks: Record<string, { correct: number; total: number }> = {}
    OPERATING_LAYER_QUESTIONS.forEach((q, i) => {
      ...
      <div className="max-w-3xl mx-auto py-8">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="bg-gradient-to-r from-teal-600 to-cyan-600 px-8 py-10 text-center">
            ...
            <div className="text-2xl text-teal-100 mb-3">{pct}% Correct</div>
            <p className="text-teal-100 text-lg">{msg}</p>
          </div>
          ...
          <div className="px-8 py-6">
                  ...
                  <div key={chunk}>
                    ...
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${p}%`,
                          background: p >= 80 ? '#16a34a' : p >= 60 ? '#ca8a04' : '#dc2626'
                        }}
                      />
                    </div>
                  </div>
            ...
            <div className="space-y-3 mb-8 max-h-96 overflow-y-auto pr-2">
              {OPERATING_LAYER_QUESTIONS.map((q, i) => {
                ...
                return (
                  <div key={q.id} className={`flex items-start gap-3 p-3 rounded-lg border ${isCorrect ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'}`}>
                    <span className={`mt-0.5 flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold text-white ${isCorrect ? 'bg-green-500' : 'bg-red-500'}`}>
                      {isCorrect ? '✓' : '✗'}
                    </span>
    ...
    <div className="max-w-3xl mx-auto py-8">
      <div className="mb-6">
        <div className="flex justify-between text-sm text-gray-500 dark:text-gray-400 mb-2">
          <span className="font-medium text-teal-600 dark:text-teal-400">{q.chunk}</span>
        ...
        <div className="flex justify-between text-xs text-gray-400 mt-1">
          <span>
            Score so far: {answers.filter((a, i) => a === OPERATING_LAYER_QUESTIONS[i].correctAnswer).length}/
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden mb-4">
        <div className="px-6 py-5 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 mb-3">
            <span className="bg-teal-600 text-white text-xs font-bold px-2.5 py-1 rounded-full">Q{q.id}</span>
          </div>
        ...
        <div className="p-6 space-y-3">
          {q.options.map(opt => {
            ...
            let cls = 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:border-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/20'
            if (confirmed) {
      ...
      <div className="flex gap-3">
        ...
        {!confirmed ? (
          <button
            ...
            disabled={!selected}
            className="flex-1 py-3 rounded-xl bg-teal-600 hover:bg-teal-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold transition-colors"
          >
            Submit Answer
          </button>
        ) : (
          <button
            onClick={handleNext}
            className="flex-1 py-3 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold transition-colors"
          >
